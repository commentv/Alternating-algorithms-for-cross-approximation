function [G,List_Matrix,error] = sublinear_iterative_refinement_rescaled_fast(M,G,List_Matrix,tau,R,l,l_tilde)
    ...
    % Gather the error only at the end for the transition diagrams
    [A_1, B_1] = matricization_Tucker(G,List_Matrix,1);
    error = norm(matricization(M,1) - A_1*B_1,'fro');
